...
module.exports = async function handler(req, res) {
  console.log("--- API Toro (DB): Consultando base de datos con especificaciones ---");
  ...
  try {
    ...
    const result = await client.query(
      `SELECT 
       ...
       FROM public.motos 
       WHERE marca = 'Toro' AND active = true 
       ORDER BY nombre ASC`
    );
    ...
    const products = result.rows.map(row => ({
      ...
      // Nuevas especificaciones Bera (vienen vacías para Toro)
    ...
    console.log(`--- API Toro (DB): Enviando ${products.length} productos con detalles homogéneos ---`);
    res.setHeader('Cache-Control', 's-maxage=3600, stale-while-revalidate');
    return res.status(200).json(products);
  } catch (error) {
    console.error("❌ Error en API Toro (DB):", error.message);
    return res.status(500).json({ error: "Error al obtener Toro" });
  } finally {
