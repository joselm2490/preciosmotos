...
module.exports = async function handler(req, res) {
  console.log("--- API Ava (DB): Consultando base de datos con especificaciones ---");
  ...
  try {
    ...
    const result = await client.query(
      `SELECT 
       ...
       FROM public.motos 
       WHERE marca = 'Ava' AND active = true 
       ORDER BY nombre ASC`
    );
    ...
    console.log(`--- API Ava (DB): Enviando ${products.length} productos con detalles homogéneos ---`);
    res.setHeader('Cache-Control', 's-maxage=3600, stale-while-revalidate');
    return res.status(200).json(products);
  } catch (error) {
    console.error("❌ Error en API Ava (DB):", error.message);
    return res.status(500).json({ error: "Error al obtener Ava" });
  } finally {
