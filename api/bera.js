  ...
  try {
    ...
    const result = await client.query(
      `SELECT 
       ...
       FROM public.motos 
       WHERE marca = 'Bera' AND active = true 
       ORDER BY nombre ASC`
    );
    
    // Mapeamos al formato WP REST API compatible
    const products = result.rows.map(row => ({
      ...
      // Nuevas especificaciones Empire (vienen vacías para Bera)
    ...
    console.error("❌ Error en API Bera (DB):", error.message);
    return res.status(500).json({ error: "Error al obtener Bera" });
  } finally {
