...
module.exports = async function handler(req, res) {
  console.log("--- API Escuda (DB): Consultando base de datos con especificaciones ---");
  ...
  try {
    ...
    const result = await client.query(
      `SELECT 
       ...
       FROM public.motos 
       WHERE marca = 'Escuda' AND active = true 
       ORDER BY nombre ASC`
    );
    ...
    const products = result.rows.map(row => ({
      ...
      // Nuevas especificaciones Bera (vienen vacías para Escuda)
      ...
      // Nuevas especificaciones Empire / TVS / Toro
      ...
      hamburger: null, // Evitando campos indeseados
    ...
    console.log(`--- API Escuda (DB): Enviando ${products.length} productos con detalles homogéneos ---`);
    res.setHeader('Cache-Control', 's-maxage=3600, stale-while-revalidate');
    return res.status(200).json(products);
  } catch (error) {
    console.error("❌ Error en API Escuda (DB):", error.message);
    return res.status(500).json({ error: "Error al obtener Escuda" });
  } finally {
